...
export class AnyVariableBranchFactoryForFiltering implements base.TreeFactoryCandidate {
  public doesApply(args: base.BranchingArgs) {
    return /* @construction */ !args.complex && args.mirroredIdFrom === undefined && args.singleValued && !args.inScopeVariable;
  }
...
export class AnyVariableBranchForFiltering extends base.Branch {
  ...
  protected applyBranch(args: base.TraversingArgs): base.BranchingResult {
    ...
    subPattern = basePattern.looseBranch(args.mapping.variables.getVariable());

    if (this.branchingArgs.inverse === true)
      subPattern = subPattern.optionalInverseBranch(propertyName, variableName);
    else
      subPattern = subPattern.optionalBranch(propertyName, variableName);
